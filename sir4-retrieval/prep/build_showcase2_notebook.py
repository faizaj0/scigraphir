...
def build(dataset: str) -> str:
    cell = open(f"{ROOT}/colab_showcase_cell.py").read()
    ...
    pick_src = open(f"{ROOT}/colab_paths_pick_cell.py").read()
    ...
    fig_cell = ("# ===== Figures: the rank ladder per pair (graph channel on/off, baselines) and the interpreted routes with gates =====\n"
                ...
                "_qt = f\"{CARGO_ROOT}/quartet/data.nosync/benchmark/{DATASET.replace('sir4_', '')}_test_final/eval.json\"\n"
    ...
    out = f"{ROOT}/colab_showcase2_{dataset}.ipynb"
