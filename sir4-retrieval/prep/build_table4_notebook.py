...
CARGO = S4.parent
ENGINE = CARGO / "kg-construction/gfm-rag"
OUT = S4 / "colab_table4_openie.ipynb"
...
UNPACK = r'''# Restore the selected corpora under /content/cargo.
FORCE_UNPACK = False
marker = Path(CARGO_ROOT) / ".unpacked.json"
...
if missing:
    cache_directory = Path(CARGO_ROOT) / "outputs/caches"
    ...
    if available:
        ...
        cargo_directory = Path(CARGO_ROOT)
...
PROJECT_SOURCES = [
    ("/content/cargo/cargo_paths.py", CARGO / "cargo_paths.py"),
    ("/content/cargo/kg-construction/eval/cargo_operator.py",
     CARGO / "kg-construction/eval/cargo_operator.py"),
    ("/content/cargo/sir4-retrieval/eval/semantic_scorer.py",
     S4 / "eval/semantic_scorer.py"),
    ("/content/cargo/kg-construction/experiments/probe_greasoner/precompute_operator_components.py",
     CARGO / "kg-construction/experiments/probe_greasoner/precompute_operator_components.py"),
    ("/content/cargo/kg-construction/experiments/probe_greasoner/precompute_semantic_components.py",
     CARGO / "kg-construction/experiments/probe_greasoner/precompute_semantic_components.py"),
]
...
def common_cells(intro, precisions):
    ...
    showcase = json.loads((S4 / "colab_showcase_all.ipynb").read_text())
...
def build():
    ...
    driver, final = (S4 / "colab_ccmp_mechanism_cell.py").read_text().split(
    ...
    cells += [md("## 6. Select checkpoints and prepare graph-aligned inputs"),
              initialization_cell(cases), code((S4 / "colab_analysis_setup_cell.py").read_text()),
