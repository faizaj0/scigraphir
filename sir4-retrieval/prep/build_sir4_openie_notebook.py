...
THE GRAPHS EXIST. kg-construction/data/sir4_<field>_{train,test}/processed/stage1 holds an
...
SETS    = {d: f"{CARGO_ROOT}/{rel}" for d, rel in __SETS_MAP__.items() if d in FIELDS}
SEEDLESS = {}
for d in FIELDS:
...
def main() -> int:
    ...
    a.out = a.out or (f"{ROOT}/colab_tomato_openie_ablation.ipynb" if a.dataset == "tomato" else f"{ROOT}/colab_sir4_openie_ablation.ipynb")
