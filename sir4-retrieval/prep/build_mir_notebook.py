...
_HELP = _HELP.replace('env = dict(os.environ, CARGO_ROOT=CARGO_ROOT, CARGO_DATASET=DATASET, PYTHONUNBUFFERED="1")',
...
for s, g in (("train", TRAIN), ("test", TEST)):
    for label, p in ((f"corpus {s}", f"{cp.corpus_dir(s)}/raw/documents.json"),
        ...
        print(f"  ok  {label:13} {p.replace(CARGO_ROOT, '<root>')}")
...
if os.path.isdir(f"{CACHE}/op_emb"):
    shutil.copytree(f"{CACHE}/op_emb", f"{CARGO_ROOT}/outputs/caches/op_emb", dirs_exist_ok=True)
...
def main() -> int:
    ...
    ap.add_argument("--graph", default="frame", choices=["frame", "openie"],
                    help="frame = mir_*_v16sc (SciAfford graph, both fusion arms); openie = mir_* "
                         "(entity+document OpenIE graph from kg-construction/run_index.sh, one arm: "
    ...
    assert '"map":' in overlay["sir4-retrieval/eval/score_sir4.py"], "score_sir4.py has no mAP; the table needs it"
    ...
    out = a.out or (f"{ROOT}/colab_mir_openie.ipynb" if a.graph == "openie" else f"{ROOT}/colab_mir_{a.ccmp}.ipynb")
