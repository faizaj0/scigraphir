...
OUT = (f"{ROOT}/{DSET}_" + (_NAME if _NAME else
...
# The bundle zip MIRRORS the repo, so unpacking it here and pointing CARGO_ROOT
...
CARGO_ROOT = "/content/cargo"                # local only, never on Drive
os.environ["CARGO_ROOT"] = CARGO_ROOT
os.environ["CARGO_DATASET"] = DATASET

# cargo_paths puts built graphs at {CARGO_ROOT}/kg-construction/data/<graph>,
...
if os.path.exists(BUNDLE):
    ...
    need = [
        f"kg-construction/data/{DATASET}_train/raw/documents.json",
        f"kg-construction/data/{DATASET}_test/raw/documents.json",
        f"kg-construction/data/{TRAIN}/processed/stage1/nodes.csv",
        f"kg-construction/data/{TEST}/processed/stage1/nodes.csv",
        # DATASET-SCOPED ON SIR-4, UNSCOPED ON TOMATO. cargo_paths.is_legacy() is
        ...
        f"kg-construction/construct_v2/cache/__PROBEDIR__probes_train.jsonl",
        f"kg-construction/construct_v2/cache/__PROBEDIR__probes_test.jsonl",
...
# snapshot that had drifted behind kg-construction/gfm-rag: multi-corpus operator
...
# encoding. So they are parked outside CARGO_ROOT and restored afterwards; a
...
# stale against the repo. Right now all four ship a cargo_operator.py,
...
# minutes into a run.
_need = {
    f"{CARGO_ROOT}/kg-construction/eval/cargo_operator.py": ["--model", "model_slug"],
    f"{CARGO_ROOT}/kg-construction/experiments/probe_greasoner/precompute_operator_components.py": ["--model"],
    ...
    f"{CARGO_ROOT}/sir4-retrieval/eval/score_sir4.py": ["--cols", "--gold-strata", "mrr_best",
                                                        "semantic-scorer comparison"],
    f"{CARGO_ROOT}/sir4-retrieval/eval/semantic_scorer.py": ["--arms", "GatedScorer",
                                                             "multigold_loss"],
}
for _f, _toks in _need.items():
    ...
    assert not _miss, (f"{os.path.basename(_f)} on this runtime lacks {_miss}. It came "
                       ...
                       f"{DRIVE}/code_overlay/" + os.path.relpath(_f, CARGO_ROOT))
...
# Resolve every input through cargo_paths rather than by hand, so a path that is
...
for s in ("train", "test"):
    for label, p in ((f"corpus {s}", f"{cp.corpus_dir(s)}/raw/documents.json"),
        ...
        print(f"  ok  {label:14} {p.replace(CARGO_ROOT, '<root>')}")
...
SETS = f"{CARGO_ROOT}/quartet/data/benchmark/__SETSDIR__/sets.json"
...
def save_cache():
    ...
    saved = []
    src = f"{CARGO_ROOT}/outputs/caches/op_emb__EMBDS__"
    if os.path.isdir(src):
        shutil.copytree(src, f"{CACHE}/op_emb", dirs_exist_ok=True); saved.append("op_emb")
    for f in (f"{CARGO_ROOT}/kg-construction/eval/operator_params__PARAMDS__{OP_SLUG}.json", BGE_PRED):
        if os.path.exists(f):
...
def restore_cache():
    ...
    src = f"{CACHE}/op_emb"
    if os.path.isdir(src):
        shutil.copytree(src, f"{CARGO_ROOT}/outputs/caches/op_emb__EMBDS__", dirs_exist_ok=True)
        got.append("op_emb")
    pj = f"{CACHE}/operator_params__PARAMDS__{OP_SLUG}.json"
    if os.path.exists(pj):
        shutil.copy(pj, f"{CARGO_ROOT}/kg-construction/eval/"); got.append("operator params")
...
env = dict(os.environ, CARGO_ROOT=CARGO_ROOT, CARGO_DATASET=DATASET,
           PYTHONUNBUFFERED="1")
...
sh(f"python3 -u eval/cargo_operator.py --dataset $CARGO_DATASET --model {OP_MODEL} "
...
#     begin from another corpus's calibration, and cargo_operator.py used to
...
for split, graph in (("train", TRAIN), ("test", TEST)):
    sh(f"python3 -u experiments/probe_greasoner/precompute_operator_components.py --model {OP_MODEL} "
       f"--dataset $CARGO_DATASET --model /content/qwen3 "
...
cells.append(code('''sh("python3 -u eval/bge_sir4.py --dataset $CARGO_DATASET --split test", S4)
...
args = (f"--pred {BGE_PRED} "
        f"--queries {CARGO_ROOT}/kg-construction/data/{DATASET}_test/raw/test.json "
...
sh(f"python3 -u eval/bge_sir4.py --dataset $CARGO_DATASET --split test "
...
# cap came from cargo_operator.py, where it fitted four parameters and more data
...
sh(f"python3 -u eval/semantic_scorer.py --dataset $CARGO_DATASET --model {OP_MODEL} "
...
QS   = f"{CARGO_ROOT}/kg-construction/data/{DATASET}_test/raw/test.json"
...
for split, graph in (("train", TRAIN), ("test", TEST)):
    sh(f"python3 -u experiments/probe_greasoner/precompute_semantic_components.py "
       f"--dataset $CARGO_DATASET --model {OP_MODEL} --graph {graph} --split {split}", KGDIR)
...
**The operator row is 5d's `current`, not `cargo_operator.py` from 5b.** 5d refits the
...
# THE SAME LEGACY RULE, THIRD TIME. cargo_operator.py:279 builds its params filename as
...
# FOURTH INSTANCE, and the expensive one. cargo_paths.emb_dir() is UNSCOPED on TOMATO
