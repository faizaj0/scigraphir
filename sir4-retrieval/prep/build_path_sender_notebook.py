...
S4 = base.S4
OUT = S4 / "colab_ccmp_path_sender_effect.ipynb"
...
Download `outputs/ccmp_path_sender/latest_*` into `sir4-retrieval/results/qualitative/ccmp_path_sender_effect/drive/` and re-render locally with `python3 eval/ccmp_path_sender_effect.py <results.json> <out_dir>`.
"""
...
def build():
    ...
    driver, final = (S4 / "colab_ccmp_path_sender_cell.py").read_text().split("# --- combined summary", 1)
    final = final.partition("\n")[2]
    cells += [base.md("## 6. Select checkpoints and prepare graph-aligned inputs"), init,
              base.code((S4 / "colab_analysis_setup_cell.py").read_text()),
