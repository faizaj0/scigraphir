...
kg-construction / G-Reasoner scripts already expect.

WHY THE OUTPUT IS CALLED `tomato_*`. Five scripts in the pipeline hardcode
`~/Desktop/CARGO/kg-construction/data/tomato_{split}/...`:

    kg-construction-v16/extract_frames.py             DATA_ROOT/tomato_{split}/raw/
    kg-construction-v16/build_greasoner_dataset.py    KG/data/tomato_{split}/raw/
    kg-construction/eval/cargo_operator.py            BASE/data/tomato_{split}/raw/
    kg-construction/eval/bge.py                       (same corpus loader)
    experiments/probe_greasoner/precompute_operator_components.py
...
`construct_v2/cache/probes_{split}.jsonl`, `kg-construction-v16/cache/frames_*`)
...
CARGO = os.path.dirname(ROOT)                                 # ~/Desktop/CARGO
BENCH = f"{CARGO}/quartet/data/benchmark"
# Where the pipeline scripts resolve corpora (cargo_paths.corpus_dir).
...
def main() -> int:
    ...
    #   1. sir4-retrieval/data/<domain>/  -- our own copy, survives anything done
    #      to kg-construction/data, and is what prep/make_smoke.py carves from.
    #   2. kg-construction/data/sir4_<domain>_<split>/  -- where cargo_paths
