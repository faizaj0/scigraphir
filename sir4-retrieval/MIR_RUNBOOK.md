...
`[cargo_paths] dataset=mir` banner on each. The staged layout (done):

```
kg-construction/data/mir_train/raw/{documents.json,train.json}   4,678 docs, 1,270 queries
kg-construction/data/mir_test/raw/{documents.json,test.json}     4,857 docs,   155 queries
```
...
```bash
cd ~/Desktop/CARGO && python3 sir4-retrieval/prep/stage_mir.py
```
...
```bash
cd ~/Desktop/CARGO/kg-construction-v16 && python3 extract_frames.py --dataset mir --side doc --split train --workers 128
```

```bash
cd ~/Desktop/CARGO/kg-construction-v16 && python3 extract_frames.py --dataset mir --side query --split train --workers 128
```
...
```bash
cd ~/Desktop/CARGO/kg-construction-v16 && cp cache/mir/frames_doc_train.jsonl cache/mir/frames_doc_test.jsonl
```

Then the test side picks up only the ~180 documents not in train:

```bash
cd ~/Desktop/CARGO/kg-construction-v16 && python3 extract_frames.py --dataset mir --side doc --split test --workers 128
```

```bash
cd ~/Desktop/CARGO/kg-construction-v16 && python3 extract_frames.py --dataset mir --side query --split test --workers 128
```

Check counts: doc frames should be 4,678 (train) and 4,857 (test); query frames 1,270 and 155.

```bash
wc -l ~/Desktop/CARGO/kg-construction-v16/cache/mir/*.jsonl
```

## 2. Probes  (PAID, small)

```bash
cd ~/Desktop/CARGO/kg-construction && python3 construct_v2/gen_probes.py --dataset mir --split train --workers 128
```

```bash
cd ~/Desktop/CARGO/kg-construction && python3 construct_v2/gen_probes.py --dataset mir --split test --workers 128
```
...
```bash
cd ~/Desktop/CARGO/kg-construction-v16 && for s in test train; do python3 build_greasoner_dataset.py --dataset mir --split $s --tau_canon 0.95 --no_entity_seeds --no_probe_seeds; done
```

## 4. Audit  (free)

```bash
cd ~/Desktop/CARGO/sir4-retrieval && for s in test train; do python3 eval/audit_graph.py --dataset mir --split $s --spread --sample 40; done
```
...
```bash
cd ~/Desktop/CARGO/sir4-retrieval && python3 prep/bundle.py --dataset mir
```
...
Upload `sir4-retrieval/mir_bundle.zip` to `MyDrive/cargo-gfmrag/`.
...
Run in `sir4-retrieval/llm_baselines/` with `OPENAI_API_KEY` exported.

```bash
cd ~/Desktop/CARGO/sir4-retrieval/llm_baselines && python3 build_moose_star_tree_sir4.py --field mir
```

```bash
cd ~/Desktop/CARGO/sir4-retrieval/llm_baselines && python3 run_moose_star_sir4.py --field mir --subset none --limit 3
```

```bash
cd ~/Desktop/CARGO/sir4-retrieval/llm_baselines && python3 run_moose_star_sir4.py --field mir --subset none --num-workers 16
```

```bash
cd ~/Desktop/CARGO/sir4-retrieval/llm_baselines && python3 score_rankings_sir4.py --field mir --subset none --rankings outputs/moose_star/mir/rankings.json --method "MOOSE-Star (gpt-4o-mini)"
```
...
```bash
cd ~/Desktop/CARGO/sir4-retrieval && python3 prep/build_llm_expansion_notebook.py --datasets mir
```

Upload and run `sir4-retrieval/llm_expansion_baselines_mir.ipynb`. Scores land in
...
```bash
cd ~/Desktop/CARGO/kg-construction && WORKERS=64 bash run_index.sh mir_train
```
...
```bash
cd ~/Desktop/CARGO/kg-construction && grep -E "Number of passages|Total number of processed" logs/index_mir_train.log
```
...
```bash
cd ~/Desktop/CARGO/kg-construction && FP=$(ls -td tmp/kg_construction/*/mir_train | head -1 | xargs dirname) && mkdir -p "$FP/mir_test" && cp "$FP/mir_train/openie_results.jsonl" "$FP/mir_test/" && echo "seeded $FP/mir_test"
```

```bash
cd ~/Desktop/CARGO/kg-construction && WORKERS=64 bash run_index.sh mir_test
```
...
```bash
cd ~/Desktop/CARGO/kg-construction && for s in train test; do python3 -c "import csv,collections,sys; csv.field_size_limit(10**7); print('$s', dict(collections.Counter(r['type'] for r in csv.DictReader(open('data/mir_$s/processed/stage1/nodes.csv')))))"; done
```
...
```bash
cd ~/Desktop/CARGO/sir4-retrieval && python3 prep/build_mir_all_notebook.py
```

Upload and run `sir4-retrieval/colab_mir_all.ipynb` top to bottom. Every block skips finished
...
sources, built from the fork: `sir4-retrieval/gfm_overlay.zip`) after the replayed blob and refuse to
...
(prep/overlay_cell.py reads sir4-retrieval/gfm_overlay.zip; regenerate that zip from kg-construction/gfm-rag first).
...
`kg-construction/data/tomato_{train,test}_hyb/`, zipped as `tomato_hyb_bundle.zip` (31 MB). Walk
...
paste-in `colab_showcase_cell.py`. Locally, `--quartet ../quartet/data/benchmark/<field>_test_final/eval.json`
...
```bash
cd ~/Desktop/CARGO/sir4-retrieval && python3 prep/build_baselines_notebook.py --dataset mir
```

Upload and run `sir4-retrieval/mir_baselines.ipynb`. Rows come out of section 7 (read the
...
```bash
cd ~/Desktop/CARGO/sir4-retrieval && python3 prep/build_mir_zeroshot_notebook.py
```

Upload and run `sir4-retrieval/colab_mir_zeroshot.ipynb`. Outputs land in
...
```bash
cd ~/Desktop/CARGO/sir4-retrieval && python3 prep/build_mir_notebook.py --graph openie
```

Upload and run `sir4-retrieval/colab_mir_openie.ipynb`. The run is
