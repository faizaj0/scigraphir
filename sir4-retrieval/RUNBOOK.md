...
**Check the `[cargo_paths] dataset=...` banner on every command.** It prints on
...
```bash
cd ~/Desktop/CARGO && python3 sir4-retrieval/prep/stage_sir4.py --domain physics
```

Verifies duplicate ids, gold-in-corpus, empty questions and documents, then
writes the corpus to both `sir4-retrieval/data/<domain>/` and
`kg-construction/data/sir4_<domain>_{train,test}/`, which is where the pipeline
...
```bash
cd ~/Desktop/CARGO/kg-construction-v16 && for s in test train; do for side in doc query; do python3 extract_frames.py --dataset sir4_physics --side $side --split $s --workers 128; done; done
```
...
```bash
cd ~/Desktop/CARGO/kg-construction && for s in test train; do python3 construct_v2/gen_probes.py --dataset sir4_physics --split $s --workers 128; done
```
...
```bash
wc -l ~/Desktop/CARGO/kg-construction-v16/cache/sir4_physics/*.jsonl ~/Desktop/CARGO/kg-construction/construct_v2/cache/sir4_physics/*.jsonl
```
...
```bash
cd ~/Desktop/CARGO/kg-construction-v16 && for s in test train; do python3 build_greasoner_dataset.py --dataset sir4_physics --split $s --tau_canon 0.95 --no_entity_seeds --no_probe_seeds; done
```
...
```bash
cd ~/Desktop/CARGO/sir4-retrieval && for s in test train; do python3 eval/audit_graph.py --dataset sir4_physics --split $s --spread --sample 40; done
```
...
```bash
cd ~/Desktop/CARGO/sir4-retrieval && python3 prep/bundle.py --dataset sir4_physics && python3 prep/build_notebook.py --domain physics --batch 1
```

The zip mirrors the repo, so Colab unzips it to `/content/cargo`, sets
`CARGO_ROOT`, and every script resolves exactly as it does locally. The bundler
...
Upload `sir4-retrieval/sir4_<domain>_bundle.zip` to `MyDrive/cargo-gfmrag/`,
...
python3 eval/table4.py --dataset sir4_cs \
  --queries ../kg-construction/data/sir4_cs_test/raw/test.json \
  --docs ../kg-construction/data/sir4_cs_test/raw/documents.json \
  --edges ../kg-construction/data/sir4_cs_test_v16sc/processed/stage1/edges.csv \
  --quartet ../quartet/data.nosync/benchmark/cs_test_final/eval.json \
