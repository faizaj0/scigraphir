...
CARGO="${CARGO_ROOT:-$HOME/Desktop/CARGO}"
KG="$CARGO/kg-construction"
V16="$CARGO/kg-construction-v16"
...
PROBF="$KG/construct_v2/cache/$DS/probes_test.jsonl"
...
# ---------------------------------------------------------------- step 0
# The pool builder writes data/researchbench/raw, but cargo_paths resolves a corpus
...
test -f "$KG/data/${DS}_test/raw/documents.json" || {
  echo "FATAL: no corpus. Run experiments/researchbench/build_researchbench_pool.py first." >&2
  exit 1; }
python3 - <<PY
import json
...
paid "cd '$KG' && python3 construct_v2/gen_probes.py --dataset $DS --split test --workers $WORKERS"
...
if [[ -f "$KG/data/$GRAPH/operator_components.npz" ]]; then echo "   already built"
  ...
  cd "$KG" && python3 experiments/probe_greasoner/precompute_operator_components.py \
