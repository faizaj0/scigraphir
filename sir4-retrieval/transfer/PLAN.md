...
(`zr` in [cargo_operator.py:182](../../kg-construction/eval/cargo_operator.py:182))
...
- `total_S` is sliced, never recomputed. It is a per-document vector
  ...
  [precompute_operator_components.py:100](../../kg-construction/experiments/probe_greasoner/precompute_operator_components.py:100)),
...
| file | change |
|---|---|
| `kg-construction-v16/build_greasoner_dataset.py` | emit `node_provenance.json` |
| `kg-construction/gfm-rag/gfmrag/models/fusion_reasoner.py` | accept per-batch components |
| `sir4-retrieval/eval/score_sir4.py` | `--per-query-out` |
| `sir4-retrieval/prep/bundle.py` | ship `transfer/` |
...
but `cargo_paths.corpus_dir(split)` resolves `data/{DATASET}_{split}`. Move it:

```bash
mv ~/Desktop/CARGO/kg-construction/data/researchbench ~/Desktop/CARGO/kg-construction/data/researchbench_test
```
...
`[cargo_paths] dataset=...` banner: if it says anything but `researchbench`, stop.
