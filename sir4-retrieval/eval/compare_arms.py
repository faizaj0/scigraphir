...
#             This is cargo_operator.py's formula, not an approximation.
...
KGDIR, S4  = f"{CARGO_ROOT}/kg-construction", f"{CARGO_ROOT}/sir4-retrieval"
...
#
# DEFINED HERE, not read from the bundle. Reading it from cargo_operator.py meant
...
try:
    ...
    _ospec = _ilu2.spec_from_file_location("_op", f"{KGDIR}/eval/cargo_operator.py")
    _op = _ilu2.module_from_spec(_ospec); _ospec.loader.exec_module(_op)
    BUNDLE_QI = _op.QWEN_QI
except Exception as e:
    print(f"could not read the bundled cargo_operator ({e})")
...
# ---- arm builders ------------------------------------------------------------
def zr(X):                        # row-wise over documents, as in cargo_operator
...
def build_qwen3(d):
    ...
    minute per domain."""
    emb = f"{CARGO_ROOT}/outputs/caches/op_emb/sir4_{d}"
