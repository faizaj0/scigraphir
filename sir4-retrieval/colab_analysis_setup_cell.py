...
def set_dataset(dataset_name):
    ...
    for key in list(os.environ):
        if key.startswith(("CCMP", "ROUTE", "STRAT_", "CQIG", "RESID_", "MISS_W",
                           "FUSION_", "SEED_")):
            os.environ.pop(key)
    os.environ["CARGO_DATASET"] = dataset_name
    env = dict(os.environ, CARGO_ROOT=CARGO_ROOT, CARGO_DATASET=dataset_name,
               PYTHONUNBUFFERED="1")
...
def prepare_components(graph_names):
    ...
    for graph_name in graph_names:
        ...
        if not semantic_components_are_current(semantic_file):
            command = [
                sys.executable, "-u",
                "experiments/probe_greasoner/precompute_semantic_components.py",
