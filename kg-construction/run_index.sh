...
#
# Overrides baked in:
#   dataset.root=<kg-construction>/data        where the raw/ inputs live
...
WORKERS="${WORKERS:-10}"
HERE="$(cd "$(dirname "$0")" && pwd)"          # kg-construction/
: "${OPENAI_API_KEY:?set OPENAI_API_KEY first}"

cd "$HERE"   # gfmrag is pip-installed (editable), so it runs from here; keeps tmp/ + outputs/ under kg-construction/
mkdir -p logs
