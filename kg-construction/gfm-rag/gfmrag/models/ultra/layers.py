...
class GeneralizedRelationalConv(MessagePassing):
    ...
    def propagate(self, edge_index, size=None, **kwargs):
        ...
        _force_unfused = os.environ.get("RSPMM_FORCE_UNFUSED", "0") == "1"
        _per_query_fused = (_w.dim() == 2 and self.message_func != "rotate"
                            and self.aggregate_func in ("sum", "mean") and not _force_unfused)
        # EDGE-GRADIENT PASSES STAY ON THE KERNEL (13 Sep 2026). A 1-D edge_weight that requires grad
        # (separate_grad=True: NBFNet path visualisation, the CCMP mechanism and path-sender workflows)
        # used to be diverted to the vendored PyG message/aggregate path below. That path receives at
        # edge_index[1] and sends from edge_index[0]; the rspmm kernel that training and inference run
        # receives at edge_index[0] and sends from edge_index[1] (rspmm.cpp: CSR rows = edge_index[0]).
        # Measured 8 Sep 2026 on the attn arm and again 13 Sep on the SIR-4 biology CCMP checkpoint:
        # the two forwards differ by up to 25 graph-score units, so every attribution taken on the
        # unfused path described a forward the model never computes. The kernel's backward returns the
        # edge-weight gradient (verified against explicit autograd), so the gradient pass now uses the
        # kernel too; path search must then walk edges from edge_index[1] to edge_index[0]
        # (scientific_paths.interpret_scientific_paths, ccmp_mechanism_core.resolve_paths).
        # RSPMM_FORCE_UNFUSED=1 restores the old diversion for the equivalence test.
        _grad_fused = (_w.dim() == 1 and _w.requires_grad and self.message_func != "rotate"
                       and not _force_unfused)
        if _force_unfused or (not _per_query_fused and not _grad_fused
                              and (_w.requires_grad or self.message_func == "rotate" or _w.dim() == 2)):
