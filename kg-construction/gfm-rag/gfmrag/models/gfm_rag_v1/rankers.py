...
class IDFWeightedTopKRanker(BaseDocRanker):
    ...
    def __call__(self, ent_pred: torch.Tensor, ent2doc: torch.Tensor) -> torch.Tensor:
        ...
        # PATCH (bf16): torch.sparse.mm has no bf16 CUDA kernel; run it in fp32 with autocast off,
        # then cast back, so the model can train in bf16.
        with torch.autocast(device_type="cuda", enabled=False):
            doc_pred = torch.sparse.mm(masked_ent_pred.float(), ent2doc.float())
        doc_pred = doc_pred.to(ent_pred.dtype)
        return doc_pred
