...
class KGConstructor(BaseGraphConstructor):
    ...
    def build_kg(self, open_ie_result_path: str) -> str:
        ...
        for row in tqdm(extracted_triples, total=len(extracted_triples)):
            ...
            # PATCH (memory): de-indented out of the `for triple` loop so each doc is appended
            # ONCE, not once per triple. Output (document2entities) is identical; this avoids the
            # ~16x-duplicated passage_info.json (796 MB -> ~50 MB) that OOM'd the 16 GB host.
