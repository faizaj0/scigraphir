# datasets' torch formatter does `from torchvision.io import VideoReader` whenever torchvision
# is in sys.modules, and transformers puts it there. Recent torchvision removed the legacy video
# API, so that import raises inside EVERY batch fetch:
#     ImportError: cannot import name 'VideoReader' from 'torchvision.io'
# It surfaces only after the Qwen3 index is built, i.e. after the expensive part of a run.
#
# The name is needed for an isinstance check against tensor data and is never instantiated
# here, so a placeholder class is enough. Deliberately not fixed with `pip install torchvision`:
# that resolves to the latest and moves torch and CUDA underneath the runtime.
#
# TOP OF THIS FILE ON PURPOSE. This module is the entry point the training subprocess runs, so
# the shim has to be in place before anything imports `datasets`.
try:
    ...
    if not hasattr(_tvio, "VideoReader"):
        ...
        class _NoVideoReader:
            def __init__(self, *a, **k):
                raise RuntimeError(
                    "torchvision video API removed; this shim exists only so the datasets "
                    "torch formatter can import the name"
                )

        _tvio.VideoReader = _NoVideoReader
except Exception:  # torchvision absent entirely: nothing to shim, and datasets will not look
    pass
