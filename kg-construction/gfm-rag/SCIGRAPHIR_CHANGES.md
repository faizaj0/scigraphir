...
| `gfmrag/models/cqig.py` | Cross-Query Informativeness Gating, an alternative gating mechanism explored during the project. Off by default and not part of the final system. |
| `gfmrag/models/fusion_reasoner_routed.py` | Per-document convex routing between semantic and graph scores. Explored variant, not part of the final system. |
| `gfmrag/models/belief_reasoner.py` | Semantic-prior reasoner (semantic score injected as the graph's initial belief). Explored variant, not part of the final system. |
...
| `gfmrag/workflow/config/gfm_reasoner/sft_training_fusion_routed.yaml`, `sft_training_belief.yaml`, `sft_training_gfmrag.yaml`, `gfm_rag/sft_training_nodefeat.yaml` | Configs for the explored variants and the GFM-RAG baseline run through the same trainer. |
...
| `gfmrag/models/gfm_rag_v1/model_nodefeat.py` | GFM-RAG variant with text node features (baseline experiments). |
