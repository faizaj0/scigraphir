"""
kg-construction-v16 — extract the SMT relational frame per document / query.

doc frame:   task, domain, contributions[{name, achieves[], overcomes[], mechanism[]}],
             depends_on[], prior_methods[{name, has_limitation[]}], task_limitations[], findings[]
query frame: task, domain, needs[], current_methods[{name, has_limitation[]}], task_limitations[]

Providers: OpenAI (default, gpt-4o-mini) or Anthropic (--provider anthropic --model claude-...).
Resumable JSONL cache. Parallel.

  python extract_frames.py --side doc   --workers 32
  python extract_frames.py --side query --workers 32
"""
...
DATA_ROOT = os.path.join(ROOT, "kg-construction", "data")
...
from cargo_paths import add_dataset_arg, banner, corpus_dir, frames_path, set_dataset  # noqa: E402
...
SYS_DOC = (
    "Extract the relational frame of a scientific paper so others can find it as an INSPIRATION for "
    "their method.\n"
...
SYS_QUERY = (
    "A researcher states a NEED (a problem, not a solution). Extract its frame so it can be matched to "
...
def gen_one(text, side, provider, model):
    """Return the frame, or {} if every attempt failed. Callers MUST NOT cache {}.
...
def main():
    ...
    # so a --dataset sir4_cs_smoke run appended 898 SIR-4 frames into the
    # TOMATO caches and then read them back as TOMATO. frames_path() is the
    ...
    cache = frames_path(a.side, a.split)
    done = set()
    ...
    print(f"[v16] {a.side}: {len(items)} items, {len(done)} cached, {len(todo)} to do "
    ...
    with open(cache, "a") as out, ThreadPoolExecutor(max_workers=a.workers) as ex:
        ...
        for f in tqdm(as_completed(futs), total=len(futs)):
            i = futs[f]
            frame = f.result()
            # An empty frame means every retry failed. Caching it would mark the
            ...
            if not frame:
                failed += 1
                continue
            out.write(json.dumps({"id": i, "frame": frame}) + "\n")
            out.flush()
        if failed:
            print(f"[v16] {failed} items failed all retries and were NOT cached -- "
                  f"rerun the same command to pick them up")
    print(f"[v16] done -> {cache}")
