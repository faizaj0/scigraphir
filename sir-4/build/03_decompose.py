...
# --- the CONTROL arm for the hypothetical answer above -------------------------------------------
...
# the hypothetical answer was simply following orders; if the distance-targeted arm returns
...
# An `insp` that is really a citation. The hypothetical answer prompts originally listed field names
...
def clean_probe_items(items: list[dict]) -> tuple[list[dict], int]:
    """Drop hypothetical answer results whose `insp` is a citation rather than an idea.
...
def decompose_paper(paper: dict, llm: DeepSeek, probe_cross: bool = False,
    ...
    if probe_cross:
        ...
        # Gate the hypothetical answer results against the SAME bar pass 1 had to clear. Without this
        # the arms are not comparable: pass 1 is gated and the hypothetical answers were raw, so part
...
def main() -> None:
    ...
    # running in another terminal truncates the file this stage is reading: a hypothetical answer run
    ...
    if a.probe_cross:
        ...
        print(f"\n  A - B = {delta:+,}")
        if delta <= 0:
            print("  -> No evidence of a distance blind spot. The targeted hypothetical answer found no"
