#!/usr/bin/env python3
"""Stage 5. Export a resolved SIR-4 split as a retrieval benchmark.
...
def main() -> None:
    ...
    for r in ok:
        ...
        for m in M:
            ...
            for x in insps:
                ...
                if did not in documents:
                    ...
                    if not x.get("found_abstract"):
                        title_only.add(did)
                meta.setdefault(did, sir4_metadata(x))
            members.append(ids)
        ...
        for x in prim:
            meta.setdefault(doc_id(x), sir4_metadata(x))
            note_delta(x)
        ...
        #
        # This is the SIR-4 evaluation. eval_primary.json below keeps the
    ...
    # eval.json is the SET-AWARE file and the SIR-4 evaluation.
