...
Qwen3 cosine -> multi-view scorer -> scorer + OpenIE entity graph (that model's fused score) -> scorer + SciAffordGraph with the
CCMP gate off (same weights) -> scorer + SciAffordGraph + CCMP (SciGraphIR). Also writes a table of R@1 / R@3 / R@5 / R@10.
...
SPEC = {"cosine": ("1  Qwen3 cosine", "#9aa0a8", ":", "D", False, 1.3),
        ...
        "frame_off": ("4  scorer + SciAffordGraph, CCMP gate off (fused; same weights as 5)", "#7fb3d5", "--", "o", False, 1.5),
        "frame_on": ("5  scorer + SciAffordGraph + CCMP = SciGraphIR (fused)", "#1f4e79", "-", "o", True, 2.0)}
...
def main():
    ...
    for s in ("cross", "same"):
        ...
        fig.text(0.02, 0.965, "SIR-4 fields: per-gold QUARTET stratum; TOMATO and MIR: query stratum. Stage 3 is a separately trained model; stages 4 and 5 share weights.", fontsize=8, color=MUTED, ha="left")
