# Historical downstream reproduction notes

[Current downstream guide](README.md)

The material below preserves the original experiment instructions and artifact references. Availability statements, costs, and timings describe those runs; they are not current availability or pricing guarantees. The launcher still depends on an external TOMATO-Star checkout.

---

# Downstream hypothesis quality on SIR-4 (downstream comparison layout, four subsets)

Same protocol as the TOMATO downstream study: composer gpt-4o-mini with the MOOSE-Star delta prompt fed the
...
Code lives in `$EXTERNAL_REPOS/TOMATO-Star/analysis/downstream/` (same package as the TOMATO run);
`DOWNSTREAM_DATASET=sir4` re-points every script at `results/downstream_sir4/`. New: `build_inputs_sir4.py`,
`downstream_table.py` (downstream comparison layout for either dataset; reproduces the TOMATO table from the existing files).
...
- **Judge reference**: TOMATO judged against the component the query's gold contributes. Here a composition made
  from a gold document is judged against that gold's own contribution text (its SIR-4 `delta`; identical to
...
```bash
pip install gdown
cd $SCIGRAPHIR_ROOT/experiments/data
...
`predictions_scigraphir_sir4_{field}_test.json` in the same folder (all are the SciGraphIR prediction-list format):
...
| SciAfford graph + CCMP, current engine (`pred_frame_ccmp`, 8 Sep) | `11w2f5iGYL8XNtyV2t3cyz08XTOXl7hZ9` | `1TRn7uihMkHlDoET6YuTEJdiNSbePx5z1` | not on Drive | not on Drive |
...
| Aug thesis-row affordance representation+CCMP (cs only located): ccmpstrong / ccmpctrl | `1DauJNHQsnyWsm0HVAce7o2l9Qr_sIW1s` / `1VudULEJfNoguoa4d8y9o9P2SYKOkzqua` | | | |
...
bash run_downstream_sir4.sh table          # downstream comparison layout, pooled + per field -> results/downstream_sir4/table__gpt-4o-batch.tex
```
