...
# 1. the SIR-4 CS no-CCMP affordance representation arm lives under outputs/ccmp_control/ (found on Drive 9 Sep), not where the spec looked;
...
# 4. TOMATO's OpenIE arm is the July v1 fusion on the OpenIE graph (handcrafted scorer scorer, no multi-view table): the engine
#    is patched in place so interpret() runs in handcrafted scorer mode, and the checkpoint's 1-feature gate is loaded as-is;
...
_EXTRA_CKPT = {("sir4_cs", "frame_nocc"): ["outputs/ccmp_control/sir4_cs/sir4_cs_fusion_qwenmlp_ccmpctrl_epoch10_b1",
               ...
               ("tomato", "openie"):      ["outputs/v1/v1_fusion_epoch20"]}   # July v1 fusion on the OpenIE graph (handcrafted scorer scorer)
for _d in SPEC:
...
def patch_engine_t4():
    """Handcrafted scorer-scorer checkpoints (the July v1 fusion) in interpret(): idempotent string patches on the
    engine files this kernel installed; the same edits live in retriever/gfm-rag (gfm_overlay.zip)."""
    import re as _re
    ...
    s = open(ft).read(); n = 0
    if 'getattr(self.model, "semantic", "mlp") == "handcrafted"' not in s:
        old = ('        sem = self.model._s_op[0].float()\n        tag, row = self.model._sem_row[str(batch["id"][0])]\n')
        new = ('        sem = self.model._s_op[0].float()\n'
               '        if getattr(self.model, "semantic", "mlp") == "handcrafted scorer":\n'
    ...
    print(f"[engine] handcrafted scorer-scorer support: {n} edits applied" if n else '[engine] handcrafted scorer-scorer support already present')
patch_engine_t4()

T4["model_env_t4"] = r"""
# --- handcrafted scorer-scorer checkpoints: no sem_ keys (jmax None) -> model.semantic=handcrafted, 1-feature gate ---------
_SEM_MODE = {"mode": "mlp"}
...
def model_env(ckpt, graph, skey, gate=None):
    ...
    if info["jmax"] is None:
        assert gate is None and not info["resp_keys"], f"{os.path.relpath(ckpt, DRIVE)}: handcrafted scorer-scorer checkpoint with a CCMP head?"
        _SEM_MODE["mode"] = 'handcrafted'
        env_ = dict(WANDB_MODE="disabled", HYDRA_FULL_ERROR="1", PYTORCH_CUDA_ALLOC_CONF="expandable_segments:True",
                    HANDCRAFTED_COMPONENTS=opc(graph), HANDCRAFTED_COMPONENTS_TEST=opc(graph), FUSION_ROUTER="cov", STRAT_TEST=QUERIES)
        for k in list(os.environ):
            if k.startswith(("CCMP", "ROUTE", "STRAT_", "CQIG", "RESID_", "MISS_W")): os.environ.pop(k)
        print(f"[handcrafted scorer-scorer checkpoint] {os.path.relpath(ckpt, DRIVE)}: no multi-view scorer; model.semantic=handcrafted, FUSION_ROUTER=cov")
        return env_, info
    _SEM_MODE["mode"] = "mlp"
...
FRAME_EDGES = f"{DATA_ROOT}/{S['affordance representation']}/processed/stage1/edges.csv"
_field = DATASET.split("_", 1)[1] if DATASET.startswith("sir4_") else DATASET
SIR4_FILE = next((p for p in [f"{DRIVE}/quartet/{_field}_test_final/eval.json", f"{DRIVE}/quartet/{DATASET}/eval.json",
                                 f"{SCIGRAPHIR_ROOT}/sir-4/data/benchmark/{_field}_test_final/eval.json",
...
if DATASET.startswith("sir4_") and not SIR4_FILE:
    raise FileNotFoundError(
        f"{DATASET}: official SIR-4 eval.json is required for verified same-/cross-field labels. "
        "Place it at one of: "
