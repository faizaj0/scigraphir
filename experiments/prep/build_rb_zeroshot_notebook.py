...
OVERLAY_REL = ["scigraphir_paths.py",
               "experiments/eval/baselines_sir4.py",
...
for label, p in (("corpus", f"{RB_RAW}/documents.json"), ("queries", QUERIES),
    ...
    print(f"  ok  {label:8} {p.replace(SCIGRAPHIR_ROOT, '<root>')}")
...
if not os.path.exists(BGE_SLICE):
    if os.path.isdir(f"{CACHE_RB}/op_emb"):
        shutil.copytree(f"{CACHE_RB}/op_emb", f"{SCIGRAPHIR_ROOT}/outputs/caches/op_emb", dirs_exist_ok=True)
...
shutil.copytree(f"{SCIGRAPHIR_ROOT}/outputs/caches/op_emb", f"{CACHE_RB}/op_emb", dirs_exist_ok=True)
...
if not (os.path.exists(SEM_CKPT_S4) and os.path.exists(SEM_POP_S4)):
    sh(f"python3 -u eval/semantic_scorer.py --dataset sir4_{SEM_WARM_DOM} --model {OP_MODEL} "
       ...
       S4, extra={"SCIGRAPHIR_DATASET": f"sir4_{SEM_WARM_DOM}"})
...
def main() -> int:
    ...
    ap.add_argument("--out", default=f"{ROOT}/notebooks/colab_rb_zeroshot.ipynb")
    a = ap.parse_args()
