...
SETS    = {d: f"{SCIGRAPHIR_ROOT}/{rel}" for d, rel in __SETS_MAP__.items()}
for d in S4_DOMS:
...
for d in EVAL_DOMS:
    ...
    for lab, tag, model, pool, ins, extra in ARMS:
        ...
        cmd += ["--out", dest]
        rc = sh(cmd, S4, extra={"SCIGRAPHIR_DATASET": ds}, check=False)
        if rc != 0:
...
for d in EVAL_DOMS:
    ...
    if not _sem_ok(semc(d, "test")):
        sh(f"python3 -u precompute/precompute_semantic_components.py "
           f"--dataset sir4_{d} --model {OP_MODEL} --graph {g} --split test", KGDIR, extra={"SCIGRAPHIR_DATASET": f"sir4_{d}"})
...
def main() -> int:
    ...
    a.out = a.out or f"{ROOT}/colab_cells/colab_sir4_zeroshot_{'+'.join(train)}_to_{'+'.join(evald)}_{a.ccmp}.ipynb"
