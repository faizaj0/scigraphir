...
OUT = _a.out or (f"{ROOT}/notebooks/mir_baselines.ipynb" if MIR
                 else f"{ROOT}/notebooks/sir4_baselines_all.ipynb" if ALL
                 else f"{ROOT}/notebooks/{DSET}_baselines.ipynb")
...
if os.path.isdir(SCIGRAPHIR_ROOT):
    shutil.rmtree(SCIGRAPHIR_ROOT)
os.makedirs(SCIGRAPHIR_ROOT, exist_ok=True)
for ds in DSETS:
    zipfile.ZipFile(BUNDLES[ds]).extractall(SCIGRAPHIR_ROOT)
...
CORPUS = {ds: f"{SCIGRAPHIR_ROOT}/retriever/data/{ds}_{SPLIT}/raw" for ds in DSETS}
for ds in DSETS:
    for f in ("documents.json", f"{SPLIT}.json"):
...
for ds in DSETS:
    print(f"\\n===================== {ds} =====================")
    os.environ["SCIGRAPHIR_DATASET"] = ds
    for lab, tag, model, pool, ins, extra in ARMS:
...
SETS = {ds: f"{SCIGRAPHIR_ROOT}/{rel}" for ds, rel in SETS_JSON.items()}
