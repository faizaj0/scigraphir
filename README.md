...
Data, graphs, caches, checkpoints and predictions are not in the repository (see
`.gitignore`); the pipeline regenerates them under `kg-construction/data/` and
`outputs/`. Graph construction and scoring run locally; training runs on one A100 through the
notebooks. `sir4-retrieval/RUNBOOK.md` gives the full sequence for one dataset; in outline:
