...
class LLMNERModel(BaseNERModel):
    ...
    def __call__(self, text: str) -> list:
        ...
        try:
            ...
            # PATCH (robustness): one query whose NER response is truncated (finish_reason="length"
            # with response_format set) makes the OpenAI SDK raise LengthFinishReasonError, which
            # otherwise aborts the entire batch. Degrade to no entities for this query and continue.
            logger.error(f"Query NER call failed ({e}); returning no entities for this query")
            return []
