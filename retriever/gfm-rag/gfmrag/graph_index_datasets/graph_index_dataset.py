...
class GraphIndexDataset:
    ...
    def process_qa_data(self, qa_data_names: list) -> None:
            ...
            with open(data_name) as fin:
                ...
                for item in data:
                    ...
                    for node in item["start_nodes"].values():
                        valid_names = [x for x in node if x in self.node2id]
                        start_nodes_names.extend(valid_names)
                        start_nodes_ids.extend([self.node2id[x] for x in valid_names])
                    ...
                    # AQC exports semantic attachment strengths. Preserve them
                    # as a fuzzy initialization instead of collapsing all
                    # quotient seeds to the same binary value. Weights never
                    # activate nodes that are absent from start_nodes, so dense
                    # document candidates in legacy weight maps remain off.
                    start_mask = entities_to_mask(start_nodes_ids, num_nodes)
                    raw_weights = item.get("start_node_weights", {})
                    for name in start_nodes_names:
                        start_mask[self.node2id[name]] = float(raw_weights.get(name, 1.0))
                    start_nodes_mask.append(start_mask)
        ...
        for raw_data_name, num_sample in zip(qa_data_names, num_samples):
            ...
            torch.save(split, processed_split_path, pickle_protocol=4)  # PATCH: >4GiB needs protocol 4
            offset += num_sample
