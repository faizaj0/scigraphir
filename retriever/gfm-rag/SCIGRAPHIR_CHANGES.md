...
| File | Purpose |
|---|---|
| `gfmrag/models/fusion_reasoner.py` | `FusionGraphReasoner`: wraps the query-conditioned `GraphReasoner`, adds the semantic channel (handcrafted scorer or the learned multi-view scorer, `semantic=handcrafted` or `semantic=mlp`), the query-adaptive fusion router (`s_fused = z_sem + gamma_q ReLU(z_graph)`, `gamma_q = softplus(gate(phi_q))`), and the **CCMP responsibility head** (one projection per layer, shared trunk, layer embedding). |
