...
class Http:
    ...
    def get(self, url: str, headers: dict | None = None) -> dict | None:
        hdr = {"User-Agent": "quartet-stage4", **(headers or {})}
        ...
        for attempt in range(3):
            ...
            try:
                ...
                with urllib.request.urlopen(urllib.request.Request(
                        url, headers={"User-Agent": "quartet-stage4"}),
                        timeout=30) as r:
                    return r.read()
            except urllib.error.HTTPError as e:
                if e.code in (400, 403, 404):
    ...
    def post(self, url: str, payload: dict, headers: dict | None = None) -> list | None:
        hdr = {"User-Agent": "quartet-stage4", "Content-Type": "application/json",
               **(headers or {})}
...
class Crossref:
    ...
    def _fetch(self, url: str) -> bytes | None:
        ...
        for attempt in range(3):
            try:
                ua = (f"quartet-stage4 (mailto:{self.mailto})" if self.mailto
                      else "quartet-stage4")
...
def domain_relation(target_field: str | None, insp_field: str | None) -> str | None:
    ...
    Field is where QUARTET's five splits sit. OpenAlex's own `domain` level would
